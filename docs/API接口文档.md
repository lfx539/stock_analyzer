...
| 数据项 | 推荐接口 | 时效性 | 说明 |
|-------|---------|--------|------|
| ROE | 同花顺(akshare) | 季度更新 | 净资产收益率，>15%卓越 |
| 营收同比增长 | 东方财富财务数据中心 | 季度更新 | 营业收入同比变化 |
| 利润同比增长 | 东方财富财务数据中心 | 季度更新 | 净利润同比变化 |
...
| 商誉占比 | 暂用默认值 | 季度更新 | 商誉/净资产，<20%为安全 |
| 审计意见 | 默认"标准无保留意见" | 年度更新 | 年报审计意见 |

**说明：**
- 风险排查已简化为状态显示（通过/高风险淘汰）
- 大部分股票质押率为0，审计意见为标准意见，区分度较低
- 主要用于排除有明显问题的股票
...
| 字段名 | 说明 |
|-------|------|
| current_stock | 当前股票的各项指标 |
| peer_stocks | 同行业其他股票列表 |
| industry_avg | 行业平均指标 |
| dividend_yield | 股息率（%） |
| pe | 市盈率 |
| pb | 市净率 |
...
**接口地址：**
```
GET /api/screen?strategy={strategy_id}
```
...
| 策略ID | 策略名称 | 筛选条件 | 适合人群 |
|--------|---------|---------|---------|
| high_dividend | 高股息策略 | 股息率≥4%，派息率30%-70%，负债率<60% | 稳健型投资者 |
| growth | 成长股策略 | ROE>15%，营收增长>0，利润增长>0 | 进取型投资者 |
| value | 价值投资策略 | PE分位数<30%，PB分位数<30%，股息率>2% | 价值投资者 |
| low_risk | 低风险策略 | 负债率<40%，现金流健康，风险排查通过 | 保守型投资者 |
...
```json
{
  ...
  "conditions": ["股息率≥4%", "派息率30%-70%", "负债率<60%"],
  "stocks": [
    {
      "code": "600028",
      "name": "中国石化",
      "match_score": 85,
      "matched_conditions": ["股息率≥4%", "派息率30%-70%", "负债率<60%"],
      "metrics": {
        "dividend_yield": 5.2,
        "payout_ratio": 55,
        "debt_ratio": 45
...
| conditions | 筛选条件列表 |
| stocks | 符合条件的股票列表 |
| match_score | 匹配得分（满分100） |
| matched_conditions | 满足的条件列表 |
...
| 2026-04-23 | 新增盈利质量数据接口文档（营收同比增长、利润同比增长） |
| 2026-04-23 | 简化风险排查为状态显示，移除市场时机（K线图已展示趋势） |
| 2026-04-24 | 新增同行业对比接口文档（GET /api/industry/{stock_code}） |
| 2026-04-24 | 新增投资策略筛选接口文档（GET /api/screen, GET /api/strategies） |
